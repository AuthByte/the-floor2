...
class ResolveTickersRequest(BaseModel):
    ...
    max_tickers: int = Field(default=8, ge=1, le=8)


class ResolveTickersResponse(BaseModel):
    tickers: List[str]
    rationale: str = ""
    direct: bool = False
...
class HedgeFundRequest(BaseHedgeFundRequest):
    ...
    execute_alpaca_paper: bool = False
...
class DebateInterjectionRequest(BaseModel):
