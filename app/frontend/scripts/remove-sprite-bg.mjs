/**
 * Remove backgrounds from sprite PNGs in public/sprites/ (Bria RMBG 2.0).
 *
 * Usage (from app/frontend):
 *   node scripts/remove-sprite-bg.mjs
 *   node scripts/remove-sprite-bg.mjs warren_buffett jim_simons
 */
...
async function main() {
  const entries = await readdir(SPRITES_DIR).catch(() => []);
  ...
  if (pngs.length === 0) {
    console.log(`No PNGs in ${SPRITES_DIR}`);
    return;
  }

  console.log(`Processing ${pngs.length} sprite PNG(s)`);
