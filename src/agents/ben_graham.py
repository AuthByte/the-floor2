...
class BenGrahamSignal(ThesisOutlookFields):
...
def ben_graham_agent(state: AgentState, agent_id: str = "ben_graham_agent"):
    ...
    for ticker in tickers:
        ...
        artifacts: list[dict] = []
        gauge = build_graham_gauge(ticker, financial_line_items, market_cap)
        if gauge:
...
def analyze_financial_strength(financial_line_items: list) -> dict:
    ...
    total_assets = _line_val(latest_item, "total_assets")
    total_liabilities = _line_val(latest_item, "total_liabilities")
    current_assets = _line_val(latest_item, "current_assets")
    current_liabilities = _line_val(latest_item, "current_liabilities")
...
def analyze_valuation_graham(financial_line_items: list, market_cap: float) -> dict:
    ...
    current_assets = _line_val(latest, "current_assets")
    total_liabilities = _line_val(latest, "total_liabilities")
    book_value_ps = _line_val(latest, "book_value_per_share")
    eps = _line_val(latest, "earnings_per_share")
    shares_outstanding = _line_val(latest, "outstanding_shares")
