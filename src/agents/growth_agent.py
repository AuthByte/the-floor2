...
def growth_analyst_agent(state: AgentState, agent_id: str = "growth_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        if len(financial_metrics) < 4:
            progress.update_status(agent_id, ticker, "Limited history — using latest metrics only")
        ...
        artifacts: list[dict] = []
        accel = build_growth_acceleration(ticker, financial_metrics)
        if accel:
            artifacts.append(accel)
