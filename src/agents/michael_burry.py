...
from src.utils.interactive_artifacts import build_burry_contrarian
...
class MichaelBurrySignal(ThesisOutlookFields):
...
def michael_burry_agent(state: AgentState, agent_id: str = "michael_burry_agent"):
    ...
    for ticker in tickers:
        ...
        if tier0_briefings_ready(state):
            ...
            insider_trades = get_insider_trades(ticker, end_date=end_date, start_date=lookback_start)
        ...
        artifacts: list[dict] = []
        contra = build_burry_contrarian(
            ticker,
            prices=prices,
            value_score=value_analysis["score"],
            max_value_score=value_analysis["max_score"],
            contrarian_score=contrarian_analysis["score"],
            max_contrarian_score=contrarian_analysis["max_score"],
        )
        if contra:
            artifacts.append(contra)
            burry_analysis[ticker]["artifacts"] = artifacts
...
def _analyze_value(metrics, line_items, market_cap):
    ...
    # Free‑cash‑flow yield — line items first, then metrics snapshot
    fcf = getattr(latest_item, "free_cash_flow", None) if latest_item else None
    fcf_yield = None
    if fcf is not None and market_cap:
        fcf_yield = fcf / market_cap
    elif latest_metrics and latest_metrics.free_cash_flow_yield is not None:
        fcf_yield = latest_metrics.free_cash_flow_yield
    elif latest_metrics and latest_metrics.free_cash_flow_per_share and market_cap:
        shares = getattr(latest_item, "outstanding_shares", None) if latest_item else None
        if shares:
            fcf_yield = (latest_metrics.free_cash_flow_per_share * shares) / market_cap
    ...
    # EV/EBIT — compute from enterprise value + operating income (metrics model has no ev_to_ebit field)
    enterprise_value = getattr(latest_metrics, "enterprise_value", None) if latest_metrics else None
    if enterprise_value is None and market_cap and latest_item is not None:
        cash = getattr(latest_item, "cash_and_equivalents", None)
        debt = getattr(latest_item, "total_debt", None)
        if cash is not None and debt is not None:
            enterprise_value = market_cap + debt - cash

    ebit = getattr(latest_item, "operating_income", None) if latest_item else None
    if enterprise_value and ebit and ebit > 0:
        ...
        details.append("EV/EBIT unavailable")
...
def _analyze_balance_sheet(metrics, line_items):
    ...
    # Yahoo / some APIs report D/E as a percentage (e.g. 45 = 0.45×)
    if debt_to_equity is not None and debt_to_equity > 10:
        debt_to_equity = debt_to_equity / 100
    if debt_to_equity is None and latest_item is not None:
        debt = getattr(latest_item, "total_debt", None)
        equity = getattr(latest_item, "shareholders_equity", None)
