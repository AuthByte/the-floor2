    ...
    for ticker in tickers:
        ...
        try:
            ...
            artifacts = attach_artifacts(
                ...
                ticker=ticker,
                state=state,
                metrics_ctx={"ticker": ticker, "prices": prices},
                reasoning_payload=technical_analysis[ticker]["reasoning"],
            )
        except Exception as exc:
            progress.update_status(agent_id, ticker, f"Chart render skipped: {exc}")
            artifacts = []
        if artifacts:
            technical_analysis[ticker]["artifacts"] = artifacts
        progress.update_status(agent_id, ticker, "Done", analysis=json.dumps(technical_analysis[ticker], default=str))
