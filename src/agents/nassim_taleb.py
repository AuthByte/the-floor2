...
class NassimTalebSignal(ThesisOutlookFields):
...
def nassim_taleb_agent(state: AgentState, agent_id: str = "nassim_taleb_agent"):
    ...
    for ticker in tickers:
        ...
        artifacts = build_taleb_artifacts(ticker, analysis_data[ticker])
        if artifacts:
            taleb_analysis[ticker]["artifacts"] = artifacts
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Write a full thesis (around 180-320 words) with concrete evidence, explicit fragility risks, and invalidation criteria. "
