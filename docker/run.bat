...
if "!COMMAND!"=="build" (
    docker build -t the-floor -f Dockerfile ..
    exit /b 0
)
...
:continue_ollama
    ...
    docker images -q the-floor 2>nul | findstr /r /c:"^..*$" >nul
    if !ERRORLEVEL! NEQ 0 (
        echo Building THE FLOOR image...
        docker build -t the-floor -f Dockerfile ..
    )

    echo Running THE FLOOR with Ollama using Docker Compose...
...
:: Add the command
set CMD=!CMD! the-floor python !SCRIPT_PATH! --ticker !TICKER! !START_DATE! !END_DATE! !INITIAL_PARAM! --margin-requirement !MARGIN_REQUIREMENT! !SHOW_REASONING!
