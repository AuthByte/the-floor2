...
if [ -n "$USE_OLLAMA" ]; then
  ...
  if [[ "$(docker images -q the-floor 2> /dev/null)" == "" ]]; then
...
# Add the command
CMD="$CMD the-floor python $SCRIPT_PATH --ticker $TICKER $START_DATE $END_DATE $INITIAL_PARAM --margin-requirement $MARGIN_REQUIREMENT $SHOW_REASONING"
